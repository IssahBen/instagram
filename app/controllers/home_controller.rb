...
class HomeController < ApplicationController
  ...
  def set_suggestions
    ...
    [current_user.followers, current_user.followings].flatten.uniq.each do |f|
      @suggestions << [f.followers, f.followings]
    end
    @suggestions = [@suggestion, User.all.sample(10)].flatten.uniq - [current_user.followings, current_user].flatten
    @suggestions = @suggestions.sample(5).compact
  end
